...
const EditClass = () => {
    ...
    useEffect(() => {
        const fetchClass = async () => {
            try {
                ...
                setMonthlyFee(classData.monthlyFee.toString());
                setDescription(classData.description || "");
                setExistingCoverPhoto(classData.coverPhoto);
            } catch (err) {
                setError(err.response?.data?.message || "Error loading class data");
            }
        };
        fetchClass();
    }, [classId]);
    ...
    const validateForm = () => {
        ...
        if (!monthlyFee) {
            ...
            errors.monthlyFee = "Monthly fee cannot exceed LKR 10,000";
        }
    ...
    const handleRemoveCoverPhoto = () => {
        setCoverPhoto(null);
        setExistingCoverPhoto(null); // Optional: If you want to allow removing the existing cover photo
    };
                                ...
                                <form onSubmit={handleSubmit} className="space-y-6">
                                    ...
                                    <div>
                                        <label className="block text-sm font-medium text-gray-700 mb-1">
                                            Upload New Cover Photo (JPEG, PNG, max 5MB)
                                        </label>
