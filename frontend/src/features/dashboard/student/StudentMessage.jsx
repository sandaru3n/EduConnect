    ...
    <Box sx={{ display: 'flex', height: '80vh', maxWidth: 1200, mx: 'auto', p: 3 }}>
              ...
              conversations.map((conv) => (
                <ListItem button key={conv.user._id} onClick={() => setSelectedUser(conv.user)}>
                  <ListItemText
                    ...
                    secondaryTypographyProps={{ noWrap: true, maxWidth: '200px' }} // Truncate long messages
                  />
                </ListItem>
              ))
            ) : (
              <Typography>No previous conversations</Typography>
            )}
          </List>
        ) : (
          <List>
      ...
      <Box sx={{ width: '70%', pl: 2 }}>
        {selectedUser ? (
              ...
              {messages.map((msg) => (
                <Paper
                  ...
                  sx={{
                    ...
                    border: msg.sender._id !== currentUserId ? '2px solid #1976d2' : 'none',
                  }}
                >
          ...
          <Typography>Select a teacher or message to start chatting</Typography>
        )}
      </Box>
    </Box>
  );
